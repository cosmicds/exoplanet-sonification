...
>
  ...
  >
    ...
    <div id="bottom-content">
      ...
      <input type="range" id="slider" value="0" min="0" max="1" step="0.005" disabled>
...
export interface SoundTestProps {
  wwtNamespace?: string;
...
const props = withDefaults(defineProps<SoundTestProps>(), {
  wwtNamespace: "sound-test",
  initialCameraParams: () => {
    return {
      raRad: 0,
      decRad: 0,
...
function setupSlider() {
  ...
  slider.oninput = (_e) => {
    ...
    const offset = value * audioBuffer.duration;
    const duration = playing.value ? undefined : 0.1;
    source = createSource();
    source.start(0, offset, duration);

    const time = minWwtTime + value * (maxWwtTime - minWwtTime);
  ...
  store.waitForReady().then(async () => {
    ...
    fetch("exoplanetSonification.wav")
      ...
      .then(async (buf) => {
        ...
        wwtRate = (maxWwtTime - minWwtTime) / audioBuffer.duration;
      })
      .then(setupSlider)
      .catch(console.error);
...
function stopSource() {
  if (source) {
    try {
      ...
      console.log("Source hadn't started");
    }
  }
}
